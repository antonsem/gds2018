...
namespace ExtraTools
{
    ...
    {
        public static bool SetComponent<T>(this Transform t, out T component, bool needWarning = false)
        {
            component = t.GetComponent<T>();
            if (component == null && needWarning)
                Debug.LogWarning(string.Format("Couldn't find a component of type {0} on {1}!", typeof(T), t.name));
